...
import { RecycleIcon, PlusCircleIcon, DocumentTextIcon, ChartPieIcon } from '../constants';
...
const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
        return (
            <div className="p-2 bg-white/90 backdrop-blur-sm border border-[var(--color-border-subtle)] rounded-md shadow-lg text-sm">
                ...
                <Card>
                    ...
                    <div className="flex justify-around items-center h-full">
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-5xl font-bold text-[var(--text-primary)] mt-1">{totalGWP.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
                            <p className="text-sm text-[var(--text-secondary)]">kg CO₂-eq</p>
                        </div>
                    </div>
                </Card>
                {latestReport && (
                    <Card>
                        <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-4">Latest Report Analysis</h3>
                        <div className="space-y-2">
                            <p className="font-bold text-lg text-[var(--color-brand-accent)] truncate">{latestReport.title}</p>
                            <p className="text-sm text-[var(--text-secondary)]">
                                By {latestReport.author} on {new Date(latestReport.createdAt).toLocaleDateString()}
                            </p>
                            <div className="flex pt-2 space-x-4">
                                <div className="flex-1 text-center border-r border-gray-200">
                                    <p className="text-sm text-[var(--text-secondary)]">GWP</p>
                                    <p className="text-2xl font-bold text-[var(--text-primary)]">{latestReport.impacts.gwp.value.toFixed(0)}</p>
                                </div>
                                <div className="flex-1 text-center">
                                    <p className="text-sm text-[var(--text-secondary)]">Circularity Score</p>
                                    <p className="text-2xl font-bold text-[var(--text-primary)]">{latestReport.circularityScore.toFixed(0)}%</p>
                                </div>
                            </div>
                        </div>
                        <Button onClick={() => onViewReport(latestReport, 'dashboard')} className="w-full mt-4">View Full Report</Button>
                    </Card>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                     ...
                     <p className="text-sm text-[var(--text-secondary)] mb-4">Top 3 materials by average recycled content.</p>
